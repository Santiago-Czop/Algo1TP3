...
class Vector:
    ...
    def norma(self):
        ...
        return math.sqrt(sum( componente**2 for componente in self))
    ...
    def __mul__(self, numero):
        ...
        """
        if type(numero) == type(1) or type(numero) == type(1.0):
            product = tuple(componente * numero for componente in self)
            return Vector(*product)

    def __rmul__(self, numero):
        """ Utilizado cuando se hace 4*vector."""
        return self.__mul__(numero)

    def __iter__(self):
        return self.coordenadas.__iter__()
